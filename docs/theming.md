...
Colors can be modified by passing a JSON object to the `WalleeTestSdk` instance. You can either completely override the theme or only change certain colors.
...
class ViewController : UIViewController, WalleeTestResultObserver {
    ...
    let paymentSdk = WalleeTestSdk (eventObserver: self)
...
Use `setAnimation` method to change the screen change animation. Currently avaliable options are: `AnimationEnum.SLIDE` and `AnimationEnum.BUBBLE`. Default value is `AnimationEnum.SLIDE`. `WalleeTestSdk.instance?.setAnimation(AnimationEnum.BUBBLE)` allows to modify the payment dialog's dark theme. ![Slide Animation](../imgs/slideAnimation.gif) ![Bubble Animation](../imgs/bubbleAnimation.gif)
